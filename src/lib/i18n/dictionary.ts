...
/**
 * Every leaf path in the dictionary, as a dotted string: 'invoice.summary.commissionDue'.
 *
 * This is what makes a typo in a key a compile error rather than a `t()` call that
 * renders its own key onto an invoice sent to a restaurant.
 */
 ...
 * so `tCount('orders.count', n)` is checked, and a base key with no plural forms
...
/**
 * `t` and `tCount` as parameter types, for the code that writes documents rather than
 * screens (lib/export, lib/finance/categories): it is handed the provider's own
 * functions instead of reading a context it has no business depending on.
 */
export type Translate = (key: MessageKey, vars?: Vars) => string;
export type TranslateCount = (key: CountKey, count: number, vars?: Vars) => string;
 ...
 * should be visibly wrong in the UI (`invoice.summary.comissionDue` is unmistakably a bug) rather
